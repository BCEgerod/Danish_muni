...
\section*{Dynamic and Adaptive Local Responsiveness}
 ...
 This is in part a result of studies using the Census of Local government which is only updated every five years, interpolating policy preferences from elections held before and after the census. This method leaves their studies open to some of the same critique that can be leveraged against cross-sectional designs. In particular, one is  not able to rule out that citizen preferences adjust to policy, either by Tiebout-sorting (i.e., liberal policies attract liberal voters) or by citizens adjusting their preferences to fit the policy position of local political elites \citep{tiebout1956pure,broockman2017causal}. In order to do so, one would need to look at whether current changes in preferences predict future changes in policy, which is difficult when the estimate of preferences measured at time $t$ is influenced by preferences at $t+1$. Second, since policy data is only available in five year increments, these studies cannot effectively speak to how long it takes for a change in preferences to affect local government policy. 
